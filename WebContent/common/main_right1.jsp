...
<%
	try {
		...
		ResultSet rs=stmt.executeQuery("select id,name,price,company_id,expression,photo from product where id between 49 and 51");
		int flag=0;
		while(rs.next()) {
			id=rs.getInt(1);
			name=rs.getString(2);
...
%>
	...
	<tr>
		<td bgcolor=#dddddd><img height=1 src="./img/blank.gif" width=1></td>
	</tr>
	<tr>
		<td>
...
<%
	try {
		...
		Statement stmt=con.createStatement();
		ResultSet rs=stmt.executeQuery("select id,name,price,photo from product where id between 9 and 11");
		int flag=0;
		while(rs.next()) {
			id = rs.getInt(1);
			name = rs.getString(2);
...
<%
	try {
		...
		Statement stmt=con.createStatement();
		ResultSet rs=stmt.executeQuery("select id,name,price,expression,photo,code from product where category between 34 and 35");
		while(rs.next()) {
			id=rs.getInt(1);	
			name=rs.getString(2);
			price=rs.getInt(3);
			expression=rs.getString(4);
